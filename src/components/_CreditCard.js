...
import creditcardutils from '../utils/credcardutils';
import _Text from './Text';
import normzer from '../utils/normalizer';
import * as COLORS from '../styles/color';
import { DeleteAddressAction } from '../redux/addressList/actions';
import { DeleteCreditCardAction } from '../redux/creditCardList/actions';
...
const _CreditCard = (props) => {
  ...
  return (
    <View
      style={{
        height: editable ? RFValue(260) : RFValue(height),
        width: width,
        backgroundColor: editable ? '#eee' : '#fff',
      }}
    >
      <FlipCard
        style={cardStyles}
      ...
      >
        <View
          style={{
            ...frontStyles,
            paddingHorizontal: editable ? RFValue(10) : 0,
            paddingTop: editable ? RFValue(10) : 0,
          }}
        >
          <View
          // style={{ height: RFValue(height), width: width }}
          >
            <ImageBackground
            ...
            >
              <View style={styles.imageContainer}>
                ...
                <_Text
                  text={creditcardutils.formatCardNumber(String(number))}
                  // textStyle={'bold'}
                  fontSize={25}
                  color={'#fff'}
                  styles={{
                ...
                >
                  <View
                    style={{ ...styles.groupContainer, marginTop: RFValue(10) }}
                  >
                    {expiration && (
                      <>
                        <_Text
                          ...
                          fontSize={10}
                          color={color}
                          styles={{ ...styles.groupLabel, textAlign: 'center' }}
                        />

                        <_Text
                          text={creditcardutils.formatCardExpiry(expiration)}
                          // textStyle={'bold'}
                          fontSize={10}
                          color={color}
                          styles={{
                ...
                >
                  <View style={{ top: RFValue(15), position: 'relative' }}>
                    <_Text text={name} fontSize={20} styles={{}} />
                  </View>
                  <View
                    style={{
